...
--Unions can be problematic, look through the execution and notice that first name an jobtitle are in the same column, same thing with salary and age
...
--You can decide to use the AS or not, You can use Aliasing to rename
...
--ALL i did here to to run the procedure and sinnce it contains a function, its not so difficult
EXEC TEST
